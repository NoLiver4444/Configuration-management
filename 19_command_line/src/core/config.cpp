...
void Config::parse_command_line(int argc, char* argv[]) {
  ...
  // Если указан конфиг файл - парсим его
...
void Config::parse_config_file() {
  ...
  // Применяем значения из конфига (если не были установлены через командную
  // строку)
  if (vfs_path == "./vfs" &&
      config_values.find("vfs_path") != config_values.end()) {
    vfs_path = config_values["vfs_path"];
  }
  if (script_path.empty() &&
      config_values.find("script_path") != config_values.end()) {
...
bool Config::config_file_exists() const {
  return !config_path.empty() && filesystem::exists(config_path);
}

bool Config::script_file_exists() const {
  return !script_path.empty() && filesystem::exists(script_path);
}